...
| 应用名称 | 版本 | 平台 | 描述 |
|----------|------|------|------|
| fn-advancedsettings | v1.0.6 | all | 管理启动设置、电源设置、SSH设置、CPU设置、DNS设置、网络设置、代理设置、设备信息、端口信息。 |
| fn-appdownload | v1.0.6 | all | 官方商店和第三方源应用下载 |
| fn-appsettings | v1.0.3 | all | 设置已安装应用的数据 |
| fn-audioplayer | v1.0.3 | all | 音频播放器应用，用于播放音频文件。支持包括 mp3、wav、flac、ogg、m4a、aac、wma、ape 等格式，会自动加载音频文件所在目录的歌词文件；支持客户端和服务端两种模式。 |
| fn-bluetooth | v1.0.1 | all | 蓝牙设备管理工具，支持配对连接音频设备、键鼠、文件传输、共享网络等功能。支持客户端模式（扫描连接）和服务端模式（接受连接）。 |
...
| fn-installer | v1.0.1 | all | 安装器，用于安装本地 FPK 应用文件。支持右键安装。 |
| fn-iVentoy | v1.0.36-1 | all | iVentoy - PXE 启动服务器，支持通过局域网网络安装操作系统。 |
...
| fn-p2s | v1.0.2 | all | 通过 fnOS 应用网关路径访问本机端口服务。 |
...
| fn-scrutiny | v1.64.0 | all | 硬盘 S.M.A.R.T 健康监控，集成 Scrutiny Web 与 Collector，提供可视化硬盘健康状态追踪。依赖 InfluxDB。 |
| fn-terminal | v1.0.12 | all | terminal (ttyd + tmux) 是一个基于 Web 的终端应用程序，允许用户通过浏览器访问和管理服务器终端会话，提供便捷的远程终端操作体验。 |
| fn-vfnOS | v1.0.3 | all | 在 fnOS 系统中创建异架构的 fnOS 虚拟机（x86_64 fnOS 下则创建 aarch64 的 fnOS 虚拟机, aarch64 fnOS 下则创建 x86_64 的 fnOS 虚拟机）。 |
